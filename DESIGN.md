...
| `target chunks` | ~140–180 | An estimate for this corpus from the structural unit counts (≈50 FAQ pairs, ≈45 refund clauses, ≈30 eligibility blocks, ≈15 YAML records, ≈45 PDF sections). The ingest command prints the real count; the eval run records it. |
...
| `MISS_DENSE_COSINE` | calibrated floor on the **raw** top cosine | Part of miss detection (below). Set by inspecting the top cosine for the abstention cases (§f cases 10–11) against the answerable cases and placing the floor between them; logged per query as `top_score` so it can be re-calibrated from traces. |
